...
void Updater::downCompleted()
{
    ...
    if (html.startsWith(QLatin1String("Version:"))) {
        ...
        if (current.isValid && updated.isValid && current < updated) {
            mApp->desktopNotifications()->showNotification(QIcon(QSL(":icons/prometheus.svg")).pixmap(48), tr("Update available"), tr("New version of Prometheus is ready to download."));
        }
    }
